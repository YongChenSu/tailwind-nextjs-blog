...
現在有正確的 `github.ref: refs/pull/58/merge`，所以故意讓 e2e test case fail，也就是讓 Test job 標記為 fail，檢查 `Rsync jest html reports` 這個 job 會不會被跳過

**驗證結果：即便前一個 job fail，後一個 job 不會被跳過**
...
在我的範例中，使用 `!cancelled()` 確保了只要工作流程沒被取消，無論測試是否通過，都會嘗試上傳測試報告。


## 其他要注意的小地方
Github Action 的介面，外層顯示執行時間為 0 秒，但實際上點開看詳細資料，表示有跑該 job，所以要注意一下並不是 0 秒就一定沒有執行。

## 修正結果
**feat/fix-prod-e2e**
```yaml
if: (!cancelled()) && github.head_ref == 'feat/fix-prod-e2e'
```
...
## 總結
`github.ref`: 會指向 merge ref 而非發 PR 的分支 (`refs/pull/58/merge`)。
`github.head_ref`: 才是分支名稱 (`feat/fix-prod-e2e`)。
